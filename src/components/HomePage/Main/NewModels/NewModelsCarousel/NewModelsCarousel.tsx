import style from './NewModelsCarousel.module.scss';
...
export const NewModelCarousel: React.FC = () => {
  ...
  const sortedToNewArray = [...products].sort((a, b) => b.year - a.year);
  ...
  return (
    <div className={style.contentContainer}>
      <div className={style.carouselContainer}>
        <h2 className={style.NewModelTitle}>Brand new models</h2>
        ...
        <ul className={style.list} ref={listRef} onScroll={scrollButton}>
          {sortedToNewArray.map(product => (
            <li className={style.item} key={product.id}>
              ...
              <p className={style.price}>${product.priceDiscount}</p>
